...
void Army::copyUnits_(const std::vector<std::shared_ptr<Unit> >& units)
{
    ...
    for(auto it = units.begin(); it != units.end(); ++it) {
		units_.push_back(std::shared_ptr<Unit>(new Unit(*(it->get()))));
    }
}
...
void Army::purge()
{
    units_.erase(std::remove_if(units_.begin(), units_.end(),[](const std::shared_ptr<Unit>& u) {
        return !(u->isAlive());
    }), units_.end());
}
...
{
    std::for_each(units_.begin(), units_.end(), [&out](const std::shared_ptr<Unit>& unit) {
        unit->save(out);
    });
}
