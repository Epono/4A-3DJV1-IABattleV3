...
std::shared_ptr<Action> AI::operator()(Unit& unit, Army& allies, Army& opponents)
{
    try {
        if(unit.getFirerate().canShoot()) {
            ...
            if(code[1] == 'D') {
                ...
                else target = &(opponents.getHighestUnit(int(code[1]-'0')));
            }
            if(target->getPosition().distance(unit.getPosition())>unit.getRange().getValue()) {
                return std::shared_ptr<Action>(new MoveAction(unit, target->getPosition()));
            } else {
                return std::shared_ptr<Action>(new ShotAction(unit, *target));
            }
        } else {
            Unit& target = opponents.getNearestUnit	(unit.getPosition());
            Point p = (2*unit.getPosition()) - target.getPosition();
            return std::shared_ptr<Action>(new MoveAction(unit, 1000*p));
        }
    } catch(std::invalid_argument& e) {
        //std::cout<<e.what()<<std::endl;
        return std::shared_ptr<Action>(new EmptyAction(unit));
    }
}
